...
SimpleForm.setup do |config|
  ...
  config.button_class = "btn btn-primary btn-md w-full rounded-box mt-4 mb-24"
  ...
  config.wrappers :vertical_form, tag: "div", class: "mb-4" do |b|
    ...
    b.use :input,
          class: "shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 bg-white/5 focus:outline-none focus:ring-0 focus:border-blue-500 text-gray-400 leading-6 transition-colors duration-200 ease-in-out", error_class: "border-red-500", valid_class: "border-green-400"
  ...
  # vertical input for check boxes
  ...
  # radio buttons stylisés en boutons (peer trick Tailwind) -> c'est claude qui a fait ça, c'est magnifique
  config.wrappers :vertical_radio_buttons,
                  item_wrapper_tag: :div,
                  item_wrapper_class: "flex-1",
                  item_label_class: "flex items-center justify-center h-12 rounded-xl border font-bold text-sm transition-all cursor-pointer border-white/10 bg-white/5 text-white/50 peer-checked:border-primary peer-checked:bg-primary/10 peer-checked:text-white",
                  tag: "div", class: "my-4 flex flex-wrap gap-3" do |b|
    b.use :html5
    b.optional :readonly
    b.wrapper :legend_tag, tag: "legend", class: "w-full text-sm font-medium ",
                           error_class: "text-red-500" do |ba|
      ba.use :label_text
    end
    b.use :input, class: "peer hidden"
    b.use :full_error, wrap_with: { tag: "p", class: "w-full block mt-2 text-red-500 text-xs italic" }
    b.use :hint, wrap_with: { tag: "p", class: "w-full mt-2 text-gray-700 text-xs italic" }
  end
  ...
  config.wrappers :vertical_file, tag: "div", class: "" do |b|
    ...
    b.optional :minlength
    b.optional :readonly
    b.use :label, class: "text-sm font-medium block", error_class: "text-red-500"
  ...
  config.wrappers :vertical_select, tag: "div", class: "my-4", error_class: "f", valid_class: "" do |b|
    b.use :html5
    b.optional :readonly
    b.use :label, class: "text-sm font-medium block mb-2", error_class: "text-red-500"
    b.use :input, class: "w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm appearance-none", error_class: "text-red-500", valid_class: "text-green-400"
    b.use :full_error, wrap_with: { tag: "p", class: "mt-2 text-red-500 text-xs italic" }
    b.use :hint, wrap_with: { tag: "p", class: "mt-2 text-gray-700 text-xs italic" }
  end
